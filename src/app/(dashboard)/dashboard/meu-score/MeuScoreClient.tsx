...
function formatVal(key: string, val: unknown): string {
  ...
  if (isPercent) {
    if (Math.abs(n) > 0 && Math.abs(n) <= 5) return `${n > 0 ? '+' : ''}${(n * 100).toFixed(2).replace('.', ',')}%`
...
function fmtValorMetrica(pilar: string, val: number): string {
  ...
  if (Math.abs(val) > 0 && Math.abs(val) <= 5) return `${(val * 100).toFixed(1).replace('.', ',')}%`
  return `${val.toFixed(1).replace('.', ',')}%`
}

function calcFaltam(pilar: string, valorMetrica: number, meta: number, tipoComp: string): number {
  let val = valorMetrica
  if (PILAR_IS_PERCENT.has(pilar) && Math.abs(valorMetrica) > 0 && Math.abs(valorMetrica) <= 5) val = valorMetrica * 100
...
function statusInfo(score: number) {
...
function EvolucaoSection({ idCarteira }: { idCarteira: string }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5">
      ...
      <p className="text-xs text-[#9CA3AF] mb-4">Histórico dos últimos meses</p>
      <div className="h-52">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ left: 0, right: 16, top: 8, bottom: 4 }}>
            ...
            <Tooltip
              formatter={(v) => {
                const n = Number(v ?? 0)
                return [n.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' pts', 'Score']
              }}
...
export default function MeuScoreClient({ resultados, dateDisplay, dataReferencia, pilaresConfig, profileNome, idCarteira }: Props) {
  const scoresByPilar = Object.fromEntries(resultados.map(r => [r.pilar_key, r]))
  const maxMap = Object.fromEntries(pilaresConfig.map(p => [p.pilar_key, p.pontos_max]))
  const metaMap = Object.fromEntries(pilaresConfig.map(p => [p.pilar_key, { meta: p.meta, unidade: p.unidade, tipo_comp: p.tipo_comp }]))

  const rawTotal = GRUPOS.flatMap(g => g.pilares).reduce((s, p) => s + (scoresByPilar[p]?.score_planilha ?? 0), 0)
  const totalScore = Math.min(rawTotal, 10)
  const st = statusInfo(totalScore)
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-[#6B7280] mt-0.5">{profileNome} · Carteira {idCarteira} · {dateDisplay}</p>
      </div>
      ...
      <div className="space-y-5">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap mb-4">
            <div>
              ...
              <p className="text-5xl font-bold" style={{ color: st.text }}>
                ...
                <span className="text-2xl font-normal text-[#9CA3AF]">/10</span>
              </p>
            </div>
            <div className="text-right flex flex-col items-end gap-2">
              <span className="inline-block px-3 py-1.5 rounded-xl text-sm font-semibold" style={{ background: st.bg, color: st.text }}>
                {st.label}
              </span>
              <p className="text-xs text-[#9CA3AF]">Meta mínima: 4,5 pts</p>
            </div>
          </div>
          <div className="w-full bg-[#F3F4F6] rounded-full h-2.5 mb-2 overflow-hidden">
            <div className="h-2.5 rounded-full transition-all duration-700" style={{ width: `${pct}%`, background: st.text }} />
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {grupo.pilares.map(pilar => {
                  ...
                  if (pilar === 'tpv' && efVm === 0) {
                    ...
                    if (atE && paE) {
                      const a = Number(atE[1]), p = Number(paE[1])
                      if (!isNaN(a) && !isNaN(p) && p !== 0) efVm = (a / p) * 100
                    }
                  }

                  const faltam = r && metaCfg ? calcFaltam(pilar, efVm, metaCfg.meta, metaCfg.tipo_comp) : null
        ...
        {/* Gráfico de evolução */}
        <EvolucaoSection idCarteira={idCarteira} />
      </div>
    </div>
  )
}
