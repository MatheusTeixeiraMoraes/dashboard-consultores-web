...
const RadarMapa = dynamic(() => import('./RadarMapa'), {
  ssr: false,
  loading: () => <div className="h-[520px] flex items-center justify-center text-sm text-[#9CA3AF]">Carregando mapa…</div>,
})
    ...
    return (
      <RadarShell>
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-14 text-center">
          ...
          <p className="font-semibold text-[#111827]">Obtendo sua localização…</p>
          <p className="text-sm text-[#6B7280] mt-1">Autorize o acesso ao GPS no navegador.</p>
        </div>
      </RadarShell>
    )
  }
    ...
    >
      ...
      <div className="bg-white rounded-2xl border border-[#E5E7EB] p-4 mb-4 space-y-3">
        <div className="flex items-center gap-3 flex-wrap">
          <span className="text-xs font-semibold text-[#6B7280]">Raio</span>
          <input type="range" min={1} max={50} value={raio} onChange={e => setRaio(Number(e.target.value))} className="flex-1 min-w-[160px] accent-[#10B981]" />
          <div className="flex items-center gap-1">
            <input type="number" min={1} max={50} value={raio} onChange={e => setRaio(Math.min(50, Math.max(1, Number(e.target.value) || 1)))} className="w-16 border border-[#E5E7EB] rounded-lg px-2 py-1 text-sm text-[#111827]" />
            <span className="text-sm text-[#6B7280]">km</span>
          </div>
          <div className="flex rounded-xl border border-[#E5E7EB] overflow-hidden ml-auto">
            {(['lista', 'mapa'] as const).map(m => (
              <button key={m} onClick={() => setViewMode(m)}
                className={`px-3.5 py-1.5 text-sm font-medium capitalize ${viewMode === m ? 'bg-[#10B981] text-white' : 'text-[#6B7280] hover:bg-[#F9FAFB]'}`}>
                {m}
              </button>
            ))}
          </div>
      ...
      {viewMode === 'mapa' ? (
        ...
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-12 text-center">
          <p className="font-semibold text-[#111827]">Nenhum cliente no raio de {raio} km</p>
          <p className="text-sm text-[#6B7280] mt-1">Aumente o raio ou ajuste os filtros.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {filtrados.map(c => {
            ...
            return (
              <div key={c.seller_id} className={`bg-white rounded-xl border p-3.5 flex items-center gap-3 ${c.selecionado ? 'border-[#10B981] ring-1 ring-[#10B981]/30' : 'border-[#E5E7EB]'}`}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="font-medium text-[#111827] truncate">{c.seller_nome || `#${c.seller_id}`}</p>
                    <span className="text-[11px] font-bold text-[#10B981] bg-[#F0FDF4] px-2 py-0.5 rounded-full whitespace-nowrap">{fmtDist(c.dist)}</span>
                    {c.selecionado && <span className="text-[11px] font-bold text-white bg-[#10B981] px-2 py-0.5 rounded-full">Selecionado</span>}
                  </div>
                ...
                {wa && (
                  <a href={wa} target="_blank" rel="noopener noreferrer" className="text-[#10B981] p-1.5" title="WhatsApp">
                ...
                <button onClick={() => toggle(c.seller_id)}
                  className={`text-sm font-semibold px-3 py-1.5 rounded-lg whitespace-nowrap ${c.selecionado ? 'bg-[#F0FDF4] text-[#10B981] border border-[#10B981]/30' : 'bg-[#10B981] text-white'}`}>
      ...
      {selecionados.size > 0 && (
        <div className="fixed bottom-0 left-60 right-0 bg-white border-t border-[#E5E7EB] px-6 py-3 flex items-center gap-3 z-30 shadow-[0_-4px_12px_rgba(0,0,0,0.04)]">
          <span className="text-sm font-semibold text-[#111827]">{selecionados.size} cliente{selecionados.size !== 1 ? 's' : ''} selecionado{selecionados.size !== 1 ? 's' : ''}</span>
          <button onClick={() => setSelecionados(new Set())} className="text-sm text-[#6B7280] hover:underline ml-auto">Limpar</button>
          <button onClick={criarRota} className="bg-[#10B981] hover:bg-[#047857] text-white text-sm font-semibold px-5 py-2 rounded-xl">Criar Rota →</button>
        </div>
      )}
    </RadarShell>
  )
...
const selCls = 'border border-[#E5E7EB] rounded-lg px-2.5 py-1.5 text-sm text-[#111827] bg-white focus:outline-none focus:ring-2 focus:ring-[#10B981]'
  ...
  return (
    <div className="pb-16">
      <div className="flex items-start justify-between gap-4 flex-wrap mb-5">
        <div>
          <h1 className="text-xl font-bold text-[#111827]">Radar de Clientes</h1>
          <p className="text-sm text-[#6B7280] mt-0.5">{contador ?? 'Clientes próximos de você agora'}</p>
        </div>
        {onAtualizarGps && (
