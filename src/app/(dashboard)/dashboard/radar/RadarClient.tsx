...
export default function RadarClient({ clientes, podeVerTodos }: Props) {
  ...
  return (
    <RadarShell onAtualizarGps={() => lerGps(true)} atualizando={geoStatus === 'loading'}>
      ...
      {/* Barra de seleção */}
      {selecionados.size > 0 && (
        <div className="fixed bottom-[var(--delegacao-h,0px)] left-0 md:left-60 right-0 glass-blur border-t border-line px-4 md:px-6 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] flex items-center gap-3 z-30 shadow-[0_-4px_12px_rgba(0,0,0,0.04)]">
